...
def evaluate(model, dataset, user_dict, args):
    ...
    for u in users:
        ...
        if not (args.least_inter_num <= len(train[u]) + 2 <= args.most_inter_num): continue
        ...
        predictions = -model.predict(torch.tensor(u).to(args.device), torch.tensor(np.array(seq)).to(args.device), np.array(item_idx))

...
parser.add_argument('--num_epochs', default=199, type=int)
...
parser.add_argument('--original_mode', action='store_true')
...
if __name__ == '__main__':
    ...
    [user_train, user_valid, user_test, usernum, itemnum, inter_mat] = dataset
    ...
    with open("data/dbook/d_user_dict.pkl", "rb") as f1:
        user_dict = pickle.load(f1)

    sampler = WarpSampler(user_train, usernum, itemnum, user_dict, batch_size=args.batch_size, maxlen=args.maxlen, n_workers=3, existing_split=args.existing_split)
    model = MetaSANRec(inter_mat, usernum, itemnum, args).to(args.device) # no ReLU activation in original SASRec implementation?
    ...
    for epoch in range(epoch_start_idx, args.num_epochs + 1):
        ...
        for step in range(num_batch): # tqdm(range(num_batch), total=num_batch, ncols=70, leave=False, unit='b'):
            ...
            u, seq, pos, neg = torch.tensor(u).to(args.device), np.array(seq), np.array(pos), np.array(neg)
